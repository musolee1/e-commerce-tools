...
# Find and replace the URL inputs section
old_pattern = r'<div className="space-y-3">\s*\{imageUrls\.map\(\(url, index\) => \(.*?\)\)\}\s*</div>'
...
new_code = '''<div className="space-y-3">
                                ...
                                {/* Collapsible additional URLs */}
                                {imageUrls.length > 1 && (
                                    <>
                                        {(urlsExpanded || imageUrls.length === 2) && imageUrls.slice(1).map((url, index) => (
                                            <div key={index + 1} className="flex gap-2">
                                                <div className="relative flex-1">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder={`Görsel ${index + 2} URL`}
                                                        className="w-full pl-11 pr-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm"
                                                    />
                                                </div>
                                                <button
